...
**Architecture:** `raw/` 디렉토리의 원본 corpus를 graphify가 `graphify-out/graph.json`으로 만든 뒤, `scripts/build-references.mjs`가 48노드 id/title 키워드로 매칭해 `references.json`을 산출한다. VibeMap(`app.jsx`)은 이 파일을 로드해 패널에 섹션을 추가한다. `data.js`는 수정하지 않으며, `references.json`이 없거나 실패해도 사이트는 정상 동작한다.
...
**Spec:** `docs/superpowers/specs/2026-04-22-graphify-knowledge-base-design.md`
...
**신규 파일:**
- `package.json` — Node ES Module 선언 + npm 스크립트. 의존성 없음.
...
- `references.json` — 초기 빈 스키마 (git 커밋)

**수정 파일:**
- `i18n.js` — `referencesTitle` 키 3개 언어 추가
- `app.jsx` — `references.json` fetch + 패널에 "근거 자료" 섹션
- `styles.css` — `.refs` 섹션 스타일
- `README.md` — 지식 수집·업데이트 워크플로우 섹션 추가

**손대지 않는 파일:** `data.js`, `sim.js`, `index.html`, `VibeMap.html`, `CNAME`
...
`"type": "module"`이 있어야 `.mjs`뿐 아니라 `.js`도 ES Module로 해석된다. 단 `data.js`·`i18n.js`·`sim.js`·`app.jsx`는 브라우저가 `<script>`로 읽으므로 Node의 module 타입과 무관하다.
...
파일 맨 아래에 CLI 진입점을 붙인다. `data.js`는 `window.VIBEMAP_DATA`를 설정하는 브라우저 스크립트라, Node에서 바로 `import` 할 수 없다. 대신 **런타임 평가**로 `window` 섀도를 만들고 읽는다.
...
async function loadVibemapData(projectRoot) {
  const dataPath = join(projectRoot, 'data.js');
...
async function main() {
  ...
  const outPath = join(projectRoot, 'references.json');
  await writeFile(outPath, JSON.stringify(output, null, 2) + '\n', 'utf8');

  const { totalSources, mappedNodes, unmappedCount } = output.stats;
  console.log(`references.json written: ${totalSources} source(s), ${mappedNodes} mapped node(s), ${unmappedCount} unmapped.`);
...
Expected:
- stdout에 `references.json written: 0 source(s), 0 mapped node(s), 0 unmapped.`
- 프로젝트 루트에 `references.json` 생성됨
- exit code 0

- [ ] **Step 3: 파일 내용 확인**

Run: `cat references.json`
Expected:
```json
{
  "version": "1",
...
```bash
git add scripts/build-references.mjs references.json
git commit -m "feat(refs): CLI entrypoint generates empty references.json on first run"
```
...
```bash
#!/usr/bin/env bash
# scripts/ingest.sh — raw/ 를 graphify로 재인덱스하고 references.json 을 새로 생성
set -euo pipefail
...
- build-references가 실행되어 `references.json`이 업데이트됨
- exit code 0
...
## Task 12: `app.jsx`에서 references.json 로드 + state

**Files:**
- Modify: `app.jsx:16` 주변 (App 함수 상단 state 영역)
...
**Files:**
- Modify: `app.jsx:496` 주변 (패널 body 내부, "연결" 섹션 뒤)
...
- Modify: `references.json` (build-references 생성)
...
```bash
git add raw/ graphify-out/ references.json
...
VibeMap의 각 노드에는 외부 자료(블로그·문서·노트)를 자동으로 연결할 수 있습니다. 원본은 `raw/`에 모이고, [graphify](https://github.com/safishamsi/graphify)가 그래프로 만든 뒤 `scripts/build-references.mjs`가 노드별 근거 자료(`references.json`)를 생성합니다. 48개 교육 노드는 `data.js`에 그대로 유지되고, 근거 자료는 슬라이드 패널에 별도 섹션으로 노출됩니다.
...
`references.json`이 없거나 손상되면 사이트는 조용히 fallback 동작하므로, 빌드 실패가 사용자 경험을 망치지 않습니다.
...
1. ✅ npm 명령 없이 그래프 시각화 정상 (index.html 그대로)
2. ✅ `scripts/ingest.sh` 실행 시 결정적 `references.json` 생성 (정렬이 score desc, url asc)
3. ✅ intent.roboco.io / roboco.io/posts/the-art-of-vibe-coding가 관련 노드에 노출 (Task 15 확인)
4. ✅ `references.json` 삭제해도 사이트 정상 (Task 12 Step 3 확인)
